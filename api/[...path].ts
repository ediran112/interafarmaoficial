...
// Static imports so Vercel's Node File Tracer includes the src/ dependencies
// in the function bundle. The handler wraps app dispatch in try/catch to
// surface any runtime error as JSON instead of an opaque cold-start crash.
...
function searchLocalInteractions(term: string) {
  ...
  const matched = INITIAL_30_INTERACTIONS.filter((item) => {
    ...
    if (words.length > 0) {
      if (words.every((w) => normA.includes(w) || normB.includes(w) || synA.some((s) => s.includes(w)) || synB.some((s) => s.includes(w)))) return true;
      if (words.some((w) => normA.includes(w) || normB.includes(w) || synA.some((s) => s.includes(w)) || synB.some((s) => s.includes(w)))) return true;
    }
    return false;
  }).map((item, idx) => ({ ...item, id: `local-${Date.now()}-${idx}` }));
  return matched;
}
