  ...
  "results": [
    {
      ...
      "affectedOrgans": ["Coração", "Fígado"]
            ...
            console.warn('gpt-4o-mini failed in server.ts, attempting fallback to gpt-3.5-turbo:', miniErr.message || miniErr);
...
Gere um JSON com as principais (3 a 6) interações medicamentosas clinicamente relevantes para o termo/combinação: "${searchTerm.trim()}".

REGRAS:
1. Se for uma combinação de medicamentos (ex: "hidroxicloroquina e fluoxetina"), inclua obrigatoriamente a interação entre os medicamentos citados ("drugA" e "drugB").
2. Saída estritamente JSON (SEM explicações extras e sem blocos de código adicionais além do JSON).
...
[
  {
    ...
    "affectedOrgans": ["Fígado", "Coração"]
      ...
      // 3. Fallback to local
      const localResults = searchLocalInteractions(searchTerm);
